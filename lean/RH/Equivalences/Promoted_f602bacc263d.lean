...
-- claim: endgame-reduced-to-disks (f602bacc263dfd425d178d2a8f09895f4c3d18a52bb7df52277641eb4981f32b)
def Claim_f602bacc263d : Prop :=
  (∀ s : ℂ, ((1 : ℝ)/2 ≤ s.re) → (s.re ≤ (77 : ℝ)/128) → ((863 : ℝ)/64 ≤ s.im) → (s.im ≤ (71 : ℝ)/5) → riemannZeta s = 0 → s.re = 1/2) → ∀ s : ℂ, riemannZeta s = 0 → s.im ≠ 0 → |s.im| ≤ (71 : ℝ)/5 → s.re = 1/2

-- BEGIN UNTRUSTED PROOF (prover: fable-loop68, proof sha256: 76d5cce4a79fd4742574e7408570af532055d5cd5a81419271be81b78c7fb981)
theorem prove_Claim_f602bacc263d : Claim_f602bacc263d :=
  by
    unfold Claim_f602bacc263d
    intro h5
    ...
    have h2 : ∀ s : ℂ, ((1 : ℝ)/2 ≤ s.re) → (s.re ≤ (13 : ℝ)/16) → ((543 : ℝ)/64 ≤ s.im) → (s.im ≤ (607 : ℝ)/64) → RH.dirichletEtaEntire s ≠ 0 := by
      intro s ha hb hc hd
      exact prove_Claim_4adb314cb39b s (by linarith) (by linarith) (by linarith) (by linarith)
...
#rh_audit_axioms prove_Claim_f602bacc263d
