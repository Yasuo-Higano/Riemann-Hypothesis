...
|---|---|---|
| Dirichlet eta | `RH.dirichletEta = (1-2^{1-s})ζ(s)` (s=1 の値 0 はアーティファクト) | s=1 を `Function.update` + 除去可能特異点で log 2 に修正し全点忠実にする (`riemannZeta_residue_one` と `(1-2^{1-s})` の一次挙動から導出可能)。級数一致 (eta-eq-alternating-halfplane) は re>1 のみ→ 0<re≤1 の条件収束和との一致は Abel 総和の形式化が必要 |
