...
-- claim: endgame-of-certificates (b59c23763c8a7acde22050d6d24a73ef0c1151aca820b3ddafb4b3290f4740be)
def Claim_b59c23763c8a : Prop :=
  (∀ s : ℂ, ((1 : ℝ)/2 ≤ s.re) → (s.re ≤ 1) → ((0 : ℝ) ≤ s.im) → (s.im ≤ (543 : ℝ)/64) → RH.dirichletEtaEntire s ≠ 0) → (∀ s : ℂ, ((1 : ℝ)/2 ≤ s.re) → (s.re ≤ (13 : ℝ)/16) → ((543 : ℝ)/64 ≤ s.im) → (s.im ≤ (607 : ℝ)/64) → RH.dirichletEtaEntire s ≠ 0) → (∀ s : ℂ, ((13 : ℝ)/16 ≤ s.re) → (s.re ≤ 1) → ((543 : ℝ)/64 ≤ s.im) → (s.im ≤ (17 : ℝ)/2) → RH.dirichletEtaEntire s ≠ 0) → (∀ s : ℂ, ((13 : ℝ)/16 ≤ s.re) → (s.re ≤ 1) → ((17 : ℝ)/2 ≤ s.im) → (s.im ≤ (607 : ℝ)/64) → (1 - 3 ^ (1 - s)) * riemannZeta s ≠ 0) → (∀ s : ℂ, ((1 : ℝ)/2 ≤ s.re) → (s.re ≤ 1) → ((607 : ℝ)/64 ≤ s.im) → (s.im ≤ (863 : ℝ)/64) → RH.dirichletEtaEntire s ≠ 0) → (∀ s : ℂ, ((1 : ℝ)/2 ≤ s.re) → (s.re ≤ (77 : ℝ)/128) → ((863 : ℝ)/64 ≤ s.im) → (s.im ≤ (71 : ℝ)/5) → riemannZeta s = 0 → s.re = 1/2) → (∀ s : ℂ, ((77 : ℝ)/128 ≤ s.re) → (s.re ≤ 1) → ((863 : ℝ)/64 ≤ s.im) → (s.im ≤ (71 : ℝ)/5) → RH.dirichletEtaEntire s ≠ 0) → ∀ s : ℂ, riemannZeta s = 0 → s.im ≠ 0 → |s.im| ≤ (71 : ℝ)/5 → s.re = 1/2

-- BEGIN UNTRUSTED PROOF (prover: fable-loop68, proof sha256: 0e36ef948f58c9bc5ccfebe20d648c46b13af0c4f15641ad6a01b6a4030562ed)
theorem prove_Claim_b59c23763c8a : Claim_b59c23763c8a :=
  by
    unfold Claim_b59c23763c8a
    intro h1 h2 h2b h3 h4 h5 h6
    ...
    have main : ∀ s : ℂ, riemannZeta s = 0 → 0 < s.im → s.im ≤ (71 : ℝ)/5 →
      ...
      · replace hre1 : s.re < 1 := not_le.mp hre1
        ...
        · rcases le_or_gt s.im ((607 : ℝ)/64) with hB | hB
          · rcases le_or_gt s.re ((13 : ℝ)/16) with hC | hC
            · exact absurd heta (h2 s hre hC (le_of_lt hA) hB)
            · rcases le_or_gt s.im ((17 : ℝ)/2) with hF | hF
              · exact absurd heta (h2b s (le_of_lt hC) (le_of_lt hre1) (le_of_lt hA) hF)
              · exact absurd (show (1 - 3 ^ (1 - s)) * riemannZeta s = 0 by rw [hz, mul_zero])
                  (h3 s (le_of_lt hC) (le_of_lt hre1) (le_of_lt hF) hB)
          · rcases le_or_gt s.im ((863 : ℝ)/64) with hD | hD
            · exact absurd heta (h4 s hre (le_of_lt hre1) (le_of_lt hB) hD)
            · rcases le_or_gt s.re ((77 : ℝ)/128) with hE | hE
...
#rh_audit_axioms prove_Claim_b59c23763c8a
